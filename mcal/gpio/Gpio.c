...
static uint32 addressMap[4] = {GPIOA_BASE_ADDR, GPIOB_BASE_ADDR, GPIOC_BASE_ADDR, GPIOD_BASE_ADDR};
...
void Gpio_SetAlternateFunction(uint8 PortName, uint8 PinNumber, uint8 AfNumber) {
    uint8 addressIndex = PortName - GPIO_A;
    GpioType* gpioDevice = (GpioType*) addressMap[addressIndex];
    ...
    if (PinNumber < 8U) {
        ...
        uint8 pinShift = PinNumber - 8U;
        gpioDevice->GPIO_AFRH &= ~(0x0FUL << (pinShift * 4U));
        gpioDevice->GPIO_AFRH |= ((uint32)AfNumber << (pinShift * 4U));
    }
}
...
uint8 Gpio_ReadPin(uint8 PortName, uint8 PinNumber) {
    ...
    uint8 addressIndex = PortName - GPIO_A;
    GpioType* gpioDevice = (GpioType*) addressMap[addressIndex];
    data = (gpioDevice->GPIO_IDR & (0x1 << PinNumber)) >> PinNumber;
    return data;
}
